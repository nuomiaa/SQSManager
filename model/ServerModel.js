...
function getServerDir(serverName) {
  return SQSERVER.localProperty.ServerDir + serverName;
}
...
module.exports.createServer = (serverName, config) => {
  ...
  if (!fs.existsSync(config.cwd)) {
    ...
    // fsextra.copySync('./template', config.cwd);
  }
  onlyServerManager.newSquadServer(serverName);
  onlyServerManager.builderSquadServer(serverName, config);
  onlyServerManager.saveSquadServer(serverName);
  return true;
};

module.exports.deleteServer = (serverName) => {
  return onlyServerManager.delSquadServer(serverName);
};

module.exports.startServer = (serverName) => {
  if (onlyServerManager.isExist(serverName)) {
    return onlyServerManager.startSquadServer(serverName);
  }
  return false;
};
...
module.exports.stopServer = (serverName) => {
  if (onlyServerManager.isExist(serverName)) {
    return onlyServerManager.stopSquadServer(serverName);
  }
  return false;
};
...
module.exports.sendCommand = (serverName, command) => {
  if (onlyServerManager.isExist(serverName)) {
    return onlyServerManager.sendSquadServer(serverName, command);
  }
  return false;
};
  ...
  try {
    for (let k in serverCollect) {
      ...
      if (server && server.isRun()) {
        ...
        if (res) {
          SQSERVER.log("[时间期限] 服务端 [", server.dataModel.name, "]", "于现在过期，正在执行关闭程序.");
    ...
    SQSERVER.error("[时间期限] 关闭服务端时出现异常，某个服务端可能未能正确关闭:", err);
  }
});
