...
router.post("/loginout", function (req, res) {
  ...
  SQSERVER.log("[ loginout ] 用户 ", req.session["username"], " 退出，会话注销");
...
SQSERVER.login._banip = 0;

function LoginRule(ip) {
  let count = SQSERVER.login[ip] || 0;
  if (count > 10) {
    //值触发一次定时器
    if (count == 11) {
      setTimeout(() => {
...
function OnlyLoginCheck(sessionID) {
  for (let k in SQSERVER.allSockets) {
    if (SQSERVER.allSockets[k].sessionID == sessionID) {
      return true;
    }
  }
  return false;
...
router.post("/login", function (req, res) {
  ...
  //登陆次数加一
  counter.plus("login");
  SQSERVER.log("用户", username, "正在尝试登录...");
  ...
  loginUser(
    ...
    (loginUser) => {
      ...
      req.session.save();
      delete SQSERVER.login[ip];
      //添加到 login 容器  注意，全部代码只能有这一个地方使用这个函数
      loginedContainer.addLogined(req.sessionID, username, loginUser.dataModel);
      SQSERVER.log("[ Login ]", "用户 ", username, " 密匙正确", "准许登录");
      response.returnMsg(res, "login/check", true);
    },
    () => {
      //密码错误记录
      SQSERVER.login[ip] ? SQSERVER.login[ip]++ : (SQSERVER.login[ip] = 1);
      //防止数目过于太大 溢出
      SQSERVER.login[ip] > 1000 ? (SQSERVER.login[ip] = 1000) : null;
      //passwordError
      ...
      SQSERVER.log("[ Login ]", "用户 ", username, " 密匙错误", "拒绝登录");
