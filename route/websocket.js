...
function isWsOnline(token) {
  for (let k in SQSERVER.allSockets) {
    let wsSession = SQSERVER.allSockets[k];
...
router.ws("/ws", function (ws, req) {
  ...
  SQSERVER.log("[ WS CREATE ] 新的 Ws 创建 会话ID: [", session_id, "]");
  ...
  if (!username || typeof username != "string" || username.trim() == "") {
    SQSERVER.warning("错误令牌的 WS 尝试建立链接 | 已经阻止", ["用户值:", username, " 令牌值:", token].join(" "));
  ...
  //唯一性检查
  if (isWsOnline(token)) {
    SQSERVER.warning("此令牌正在使用 | 阻止重复使用 | isWsOnline", ["用户值:", username, " 令牌值:", token].join(" "));
    ws.close();
    return;
  }
  ...
  if (!loginedContainer.isLogined(session_id, username)) {
    SQSERVER.warning("未经过登陆逻辑的用户尝试连接 | 已经阻止", ["用户值:", username, " 令牌值:", token].join(" "));
    counter.plus("notPermssionCounter");
    ws.close();
    return;
  }
  ...
  //Session 级别验证登录检查
  if (!WsSession.login) {
    SQSERVER.warning("不明身份者建立 ws 链接", "已经阻止 | 可能的用户值: " + WsSession.username);
    counter.plus("notPermssionCounter");
    ws.close();
    return;
  }
  ...
  //放置全局在线列表
  SQSERVER.allSockets[uid] = WsSession;
  if (!SQSERVER.onlineUser[WsSession.username]) SQSERVER.onlineUser[WsSession.username] = WsSession;

  //检查通过..
  SQSERVER.log("[ WebSocket INIT ]", " 用户 ", username, " 与服务器建立链接");
  ...
  ws.on("message", function (data) {
    try {
      ...
      if (!WsSession.login) {
        //触发这里代表极为有可能有人正在攻击你
        SQSERVER.warning("没有登录的用户正在尝试发送 Ws 命令", "已经阻止 | 可能的用户值: " + username);
        counter.plus("notPermssionCounter");
        ws.close();
        return;
      }
      ...
      //在线用户容器检查
      if (!SQSERVER.onlineUser[username]) {
        counter.plus("userOnlineCounter");
        SQSERVER.onlineUser[username] = WsSession;
      }
      ...
      SQSERVER.error("WebSocket 请求处理时异常:", err);
    }
  });
  ...
  var HBMask = setInterval(() => {
    // 超过指定次数不响应，代表链接丢失
    if (wsAliveHBCount <= 0) {
      SQSERVER.log("[ WebSocket HBPackage ]", "用户", username, "长时间未响应心跳包 | 已自动断开");
      WebSocketClose();
    }
    wsAliveHBCount--;
  }, 1000 * 10);
  ...
  function WebSocketClose() {
    ...
    //释放全局变量
    if (SQSERVER.onlineUser[username]) {
      delete SQSERVER.onlineUser[username];
    }
    delete SQSERVER.allSockets[uid];
    SQSERVER.log("[ WebSocket CLOSE ]", "用户", username, "已经断开链接");
  }
});
