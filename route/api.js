...
router.all("/status/:name", function (req, res) {
  if (SQSERVER.localProperty.allow_status_api) {
    res.send("管理员禁止此项功能 | Access denied");
    return;
  }
  let params = req.params || {};
  let serverName = params.name || "";
  let sqserver = serverModel.ServerManager().getServer(serverName.trim());
  if (sqserver == null) {
    res.send("null");
    return;
  }
  ...
  // 取缓存资料
  const SQPingResult = SQPingProtocol.QuerySQPingTask(serverName);

  // 判断服务器启动状态发送不同的数据
  if (sqserver.isRun() && SQPingResult) {
    sendStatus = {
      id: serverName,
      lastDate: sqserver.dataModel.lastDate,
      status: sqserver.isRun(),
      serverName: SQPingResult.name,
      current_players: SQPingResult.current_players,
      max_players: SQPingResult.max_players,
      map: SQPingResult.map,
      version: SQPingResult.raw.version
    };
  } else {
    sendStatus = {
      id: serverName,
...
router.all("/start_server/:name", function (req, res) {
  ...
  try {
    ...
    // 启动服务器
    const result = serverModel.ServerManager().startSquadServer(name);
    // 返回状态码
    result ? apiResponse.ok(res) : apiResponse.error(res);
  } catch (err) {
    apiResponse.error(res, err);
...
// 更新服务器 | API
router.all("/update_server/:name", function (req, res) {
  // 用户权限判定
  if (!keyManager.hasServer(apiResponse.key(req), req.params.name)) {
    apiResponse.forbidden(res);
    return;
  ...
  // 流量限制 | 10 分钟间隔
  if (!requestLimit.execute(apiResponse.key(req) + "update_server", 1000 * 60 * 10)) {
    apiResponse.unavailable(res);
    return;
  }
  try {
    ...
    const name = req.params.name;
    // 启动服务器
    const result = serverModel.ServerManager().updateServer(name);
    // 返回状态码
    result ? apiResponse.ok(res) : apiResponse.error(res);
  } catch (err) {
    apiResponse.error(res, err);
...
router.all("/stop_server/:name", function (req, res) {
  ...
  try {
    ...
    const name = req.params.name;
    // 启动服务器
    const result = serverModel.ServerManager().stopSquadServer(name);
    // 返回状态码
    result ? apiResponse.ok(res) : apiResponse.error(res);
  } catch (err) {
    apiResponse.error(res, err);
...
router.post("/execute/", function (req, res) {
  ...
  try {
    ...
    // 启动服务器
    const result = serverModel.ServerManager().sendSquadServer(params.name, params.command);
    // 返回状态码
    result ? apiResponse.ok(res) : apiResponse.error(res);
  } catch (err) {
    apiResponse.error(res, err);
