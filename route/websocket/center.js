...
//init 记录器
SQSERVER.logCenter.initLogData("CPU", 24);
SQSERVER.logCenter.initLogData("RAM", 24);
...
setInterval(function () {
  ...
  osUtils.cpuUsage(function (v) {
    ...
    SQSERVER.dataCenter.cacheCPU = cacheCPU;
  });
  let onliec = 0;
  let sockec = 0;
  let banipc = 0;
  //统计在线用户
  for (let k in SQSERVER.onlineUser) {
    if (SQSERVER.onlineUser[k] == null) continue;
    onliec++;
  }
  //统计在线 Ws
  for (let k in SQSERVER.allSockets) {
    if (SQSERVER.allSockets[k] == null) continue;
    sockec++;
  }
  //统计封号ip数量
  for (let k in SQSERVER.login) SQSERVER.login[k] > 10 ? banipc++ : banipc;
  ...
  cacheSystemInfo = {
    ...
    loginCounter: counter.get("login") || 0, //登陆次数
    banip: banipc, //封的ip
    ...
    verisonC: mversion.verisonC,
    system: mversion.system
  };

  let useMemBai = ((os.freemem() / os.totalmem()) * 100).toFixed(0);
  //压入记录器
  SQSERVER.logCenter.pushLogData("CPU", tools.getMineTime(), parseInt(cacheCPU));
  SQSERVER.logCenter.pushLogData("RAM", tools.getMineTime(), 100 - useMemBai);

  setTimeout(() => counter.save(), 0); //让其异步地去保存
}, SQSERVER.localProperty.data_center_times);
