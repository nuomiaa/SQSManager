...
WebSocketObserver().listener("server/get", (data) => {
  ...
  let SQSERVER = serverModel.ServerManager().getServer(serverName);
  if (SQSERVER == null) {
    response.wsMsgWindow(data.ws, "服务端 " + serverName + " 不存在！请刷新或自行检查。");
    return;
  }

  let serverData = SQSERVER.dataModel;
...
WebSocketObserver().listener("server/rebulider", (data) => {
  ...
  */
  if (oldServerName != newServerName) {
    ...
    //serverModel.loadALLSquadServer();
  } else {
...
WebSocketObserver().listener("server/delete", (data) => {
  ...
  if (sqserver.isRun()) {
    response.wsMsgWindow(data.ws, "删除服务器失败: 请先关闭服务器再删除!");
    return;
  }
  ...
  try {
    ...
    response.wsMsgWindow(data.ws, "删除服务器成功√");
  } catch (e) {
  ...
  try {
    ...
    for (let k in servers) {
      try {
        ...
        SQSERVER.error("批量开启某服务器失败:", serverErr);
      }
    }
