...
WebSocketObserver().listener("userset/ServerDir", (data) => {
  response.wsSend(data.ws, "userset/ServerDir", {
    ServerDir: SQSERVER.localProperty.ServerDir
  });
});
...
WebSocketObserver().listener("userset/create", (data) => {
  ...
  try {
    ...
    SQSERVER.info("用户", data.WsSession.username, "建立", username, "用户");
    response.wsSend(data.ws, "userset/create", true);
    response.wsMsgWindow(data.ws, "用户建立完成√");
    return;
  } catch (e) {
    SQSERVER.error("用户建立失败", e);
...
WebSocketObserver().listener("userset/delete", (data) => {
  ...
  try {
    ...
    SQSERVER.info("用户", data.WsSession.username, "删除", username, "用户");
    deleteUser(
      username,
      () => {
        userCenter().initUser();
