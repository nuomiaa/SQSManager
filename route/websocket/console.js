...
serverModel.ServerManager().on("error", (data) => {
  SQSERVER.infoLog("Error".red, "[" + data.serverName + "] >>> 异常", true);
...
serverModel.ServerManager().on("exit", (data) => {
  SQSERVER.log("[" + data.serverName + "] >>> 进程退出");
  let server = serverModel.ServerManager().getServer(data.serverName);
  if (server.dataModel.autoRestart) {
    //自动重启
    setTimeout(() => {
      if (!server.isRun()) serverModel.startServer(data.serverName);
    }, 5000);
    selectWebsocket(data.serverName, (socket) => response.wsMsgWindow(socket.ws, "检测到服务器关闭，稍后将自动重启"));
    return;
  }
  //输出到标准输出
...
serverModel.ServerManager().on("open", (data) => {
  SQSERVER.log("[" + data.serverName + "] >>> 进程创建");
  // 传递开启服务端事件
...
WebSocketObserver().listener("server/console/ws", (data) => {
  ...
  if (permssion.isCanServer(userName, serverName)) {
    SQSERVER.log("[" + serverName + "] >>> 准许用户 " + userName + " 控制台监听");
  ...
  SQSERVER.log("[" + serverName + "] 拒绝用户 " + userName + " 控制台监听");
...
WebSocketObserver().listener("server/console/remove", (data) => {
  ...
  for (let k in SQSERVER.allSockets) {
    if (SQSERVER.allSockets[k].uid == data.WsSession.uid) {
      SQSERVER.allSockets[k]["console"] = undefined;
      return;
    }
  }
});
...
setInterval(() => {
  for (const serverName in consoleBuffer) {
    try {
      ...
      SQSERVER.log("实例", serverName, "日志周期性广播任务错误:");
      console.log(error);
      continue;
    }
  }
}, SQSERVER.localProperty.console_send_times);
//控制台标准输出流
