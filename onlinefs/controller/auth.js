...
router.all("/auth_master/pwd", (req, res) => {
  ...
  //基础检查
  if (!userName) {
    res.send("[ 权限阻止 ] 您未登录 ErrorCode:-1");
    return;
  }

  //统一登录逻辑性检查
  if (!loginedContainer.isLogined(req.sessionID, userName)) {
    res.send("[ 权限阻止 ] 您未登录 ErrorCode:-2");
    return;
  }

  // 判断是否为管理员
  if (permission.IsSessionMaster(req, res)) {
    SQSERVER.log("[Online Fs]", "管理员", userName, "访问服务端存放目录");
...
router.all("/auth/:servername", (req, res) => {
  ...
  if (!serverName || !userName) {
    res.send("[ 权限阻止 ] 您未登录 ErrorCode:-3");
    return;
  }

  //统一登录逻辑性检查
  if (!loginedContainer.isLogined(req.sessionID, userName)) {
    res.send("[ 权限阻止 ] 您未登录 ErrorCode:-4");
    return;
  }
  ...
  else cwd = dataModel.cwd + '/SquadGame/ServerConfig';
  SQSERVER.log("[Online Fs]", "用户", userName, "访问服务器", serverName, "根:", cwd);
