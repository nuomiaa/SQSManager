...
(function () {
  ...
  RES.getToken = function (callback) {
    ...
    var _url = SQSERVER.URL("./token?_LoveYouMaster_Time=" + Date.parse(new Date()));
    $.get(_url, function (data) {
      data = JSON.parse(data);
      if (data.hasOwnProperty("ResponseValue")) {
        SQSERVER.username = data["ResponseValue"].username;
  ...
  RES.ajax = function (parameter) {
    ...
    $.ajax({
      ...
      url: SQSERVER.URL(parameter["url"]),
  ...
  RES.redirectHTML = function (url, key, body, callback) {
    ...
    var _url = SQSERVER.URL("./public/" + url);
