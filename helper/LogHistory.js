...
class LogHistory {
  ...
  writeLine(text = "") {
    ...
    if (fs.existsSync(this.path)) {
      ...
      fs.writeFile(this.path, text, (err) => {
        if (err) SQSERVER.log("实例", this.id, "日志历史记录文件创建错误:", err.message);
      });
    }
  }
  ...
  readLine(demander = "", size = 1024, callback = () => {}) {
    ...
    fs.open(this.path, "r", (err, fd) => {
      if (err) {
        SQSERVER.log("实例", this.id, "日志历史记录文件打开错误:", err.message);
        fs.closeSync(fd);
        return;
      }
      // 计算末尾读取的指针位置
      ...
      fs.read(fd, buffer, 0, size, endReadPoint, (err, bytesRead, buffer) => {
        if (err) {
          SQSERVER.log("实例", this.id, "日志历史记录文件读取错误:", err.message);
          fs.closeSync(fd);
          return;
        }
